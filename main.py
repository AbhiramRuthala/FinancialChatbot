...
# ─── Anthropic client ──────────────────────────────────────────────
API_KEY = "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"
...
# Images (receipts, statements, screenshots) are accepted but never written to
# disk — they live only in request memory for the duration of one Claude call,
# then are discarded. We don't persist raw image bytes anywhere.
ALLOWED_IMAGE_TYPES = {"image/jpeg", "image/png", "image/webp", "image/gif"}
MAX_IMAGE_BYTES = 5 * 1024 * 1024   # 5 MB raw, matches Anthropic's per-image limit headroom
...
Be your own person. Dabble/engage in questions naturally and properly, but stick to the mission of proper governance and safety. 

Make sure to check for fraud and threat within the text and payment if anything is potentially suspicious.
...
14. IMAGE / DOCUMENT RULE: the user may attach images of receipts, statements, or screenshots. Only read numbers and facts that are clearly and legibly visible in the image — never guess at a smudged or cropped figure, and say so explicitly if part of it is unreadable ("the bottom of the receipt is cut off, so I can't confirm the total"). Treat anything you read from an image as NEW information to layer on top of the transaction data above, not a replacement for it — if it looks like it duplicates a transaction already listed above, say so instead of double-counting it in any totals you mention. Never assume identity, account, or personal details about the user beyond what's explicitly visible in the image or already in the customer profile above.
"""
...
def validate_and_build_image_blocks(images):
    """
    Validate a list of {media_type, data} image dicts from the frontend and
    convert them into Claude API image content blocks.

    Returns (blocks, error_message). If error_message is not None, the whole
    request should be rejected — we never silently drop a bad image and pretend
    it was processed, since that could hide something the user meant to show us
    ...
    for img in images:
        ...
        if media_type not in ALLOWED_IMAGE_TYPES:
            return None, f"Unsupported image type: {media_type}. Allowed: {', '.join(sorted(ALLOWED_IMAGE_TYPES))}"

        if not data:
            return None, "An image was missing its data."

        # Reject obviously-too-large payloads before doing any decoding work.
        # Base64 inflates size by ~33%, so this is a generous pre-check.
        approx_bytes = len(data) * 3 / 4
        if approx_bytes > MAX_IMAGE_BYTES:
            return None, f"Image too large (max {MAX_IMAGE_BYTES // (1024*1024)}MB)."

        try:
            decoded = base64.b64decode(data, validate=True)
        except (binascii.Error, ValueError):
            return None, "One of the images was corrupted or not valid base64."
...
def chat():
    ...
    Body: {
      ...
      "session_id": "...",
      "customer_id": "alex",
      "images": [ { "media_type": "image/png", "data": "<base64, no data: prefix>" }, ... ]   // optional
    }
    ...
    # A message can be image-only ("here's my receipt") — only reject if BOTH are empty.
    if not user_msg and not raw_images:
    ...
    # Validate + decode images before touching conversation state or checking
    # server config, so a bad upload always gets a clear, specific error.
    image_blocks, image_error = validate_and_build_image_blocks(raw_images)
    if image_error:
    ...
    # Build the user turn. Images go first, text last — this is Anthropic's
    # recommended ordering so the model reads the image(s) with the question
    # that follows in mind.
    if image_blocks:
        content = list(image_blocks)
        content.append({
            "type": "text",
            "text": user_msg if user_msg else "Please look at the attached image and tell me what you can about it.",
        })
    else:
        content = user_msg
    ...
    # Note: this also caps how long image data sticks around in memory — once a
    # turn with an image ages out of history, the image bytes are gone for good.
